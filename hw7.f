...
C-------------------------------------------------------------
      PARAMETER (NX=2600,NY=2600,NXY=NX*NY,NPHI=4)
...
C
     ...
     *        (UO(IJ),IJ=1,NIJ),(VO(IJ),IJ=1,NIJ),(TO(IJ),IJ=1,NIJ)
           ...
           OPEN(UNIT=99,FILE="solution_"//trim(FILGR)//'.TEC')
      WRITE(99,*)'VARIABLES ="X" "Y" "U" "V" "P" "PHI" "T" '
      WRITE(99,*) 'ZONE T= "NUMERICAL", I=',NI,', J=',NJ,', F= POINT'
        DO I = NI, 1, -1
        DO J = NJ, 1, -1
            K=LI(I)+J
            WRITE(99,'(6ES15.4)')X(I),Y(J),U(K),V(K),P(K),PP(K),T(K)
        ENDDO
        ENDDO
      CLOSE(99)

      OPEN(UNIT=99,FILE='Q_'//trim(FILGR)//'.plt')
      WRITE(99,*)'VARIABLES = "q" "Y"'
      WRITE(NAME,*)trim(FILGR)//"x"//trim(FILGR)
      write(99,*)'ZONE DATAPACKING=POINT, T="'//TRIM(NAME)//'"'
        DO J=2,NJM
          IJ=LI(1)+J
          S=0.5*(R(J)+R(J-1))
          D=VISC*PRR*S/(XC(2)-XC(1))
          QW=QWALL+D*(T(IJ+NJ)-T(IJ))
          WRITE(99,*)-QW,Y(J)
        END DO
...
C
        ...
        END DO
C
C.....INITIAL VARIABLE VALUES (INITIAL CONDITIONS)
C
      DO I=2,NIM
        DO IJ=LI(I)+2,LI(I)+NJM
          U(IJ)=UIN
...
C
  ...
  601 FORMAT(1H1,//,10X,A80,/,10X,50('*'),/,10X,
     ...
     *        ' DYNAMIC VISCOSITY:  ',1P1E10.4)
      ...
      WRITE(2,*) '  '
      WRITE(2,*) '          UNDER RELAXATION  FACTORS'
      WRITE(2,*) '          ========================'
